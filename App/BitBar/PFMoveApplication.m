...
#define kStrMoveApplicationCouldNotMove _I10NS(@"Could not move to Applications folder. Try moving it manually.")
...
#define kStrMoveApplicationQuestionInfoMustDoThis _I10NS(@"Dockerinfo won't function correctly unless it is in the Application Folder")
...
void PFMoveToApplicationsFolderIfNecessary(void) {
	...
	NSAlert *alert = [[[NSAlert alloc] init] autorelease];
	{
		...
		else if (IsInDownloadsFolder(bundlePath)) {//FRAK
			// Don't mention this stuff if we need authentication. The informative text is long enough as it is in that case.
			informativeText = [informativeText stringByAppendingString:@" "];
			informativeText = [informativeText stringByAppendingString:kStrMoveApplicationQuestionInfoMustDoThis];
		}
    else
    {
      informativeText = [informativeText stringByAppendingString:@" "];
